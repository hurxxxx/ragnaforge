...
## 🚀 현재 우선순위 작업 (2025-07-25 업데이트)

### ✅ 완료된 작업

#### 1. OpenAI 호환 임베딩 API 개선 - `/v1/embeddings` (완료)
- [x] **OpenAI API 스펙 완전 준수**
  - [x] 요청/응답 형식 완전 호환 (input, model, encoding_format, dimensions, user 필드)
  - [x] 배치 처리 및 토큰 제한 처리 개선
  - [x] usage 필드 정확한 토큰 계산 (prompt_tokens, total_tokens)
- [x] **에러 코드 및 메시지 표준화**
  - [x] OpenAI API와 동일한 에러 코드 및 메시지 형식
  - [x] 400, 401, 429, 500 등 표준 HTTP 상태 코드 적용
  - [x] 에러 응답 구조 표준화 (error.type, error.message, error.code)
- [x] **API 안정성 강화**
  - [x] 강화된 입력 검증 (빈 입력, 타입 검증, 길이 제한)
  - [x] 모델 로딩 안전성 (건강성 검사, GPU 메모리 관리)
  - [x] 임베딩 생성 안정성 (NaN/무한값 검증, 출력 검증)
  - [x] 동시 접근 안정성 (멀티스레드 환경 테스트 완료)

### 🔶 HIGH PRIORITY

#### 2. 문서 변환 시스템 완성 - Docling/Marker 통합 (구현도: 85%)
- [x] **Docling 변환 엔진 완성** (완성도: 95%)
  - [x] PDF 변환 기본 기능 구현
  - [x] 이미지 추출 및 메타데이터 추출
  - [x] GPU 메모리 최적화
  - [x] DOCX, PPTX, XLSX Office 문서 지원 완성
  - [x] 다중 형식 지원 (PDF, DOCX, PPTX, XLSX)
  - [x] 범용 문서 변환 메서드 구현
  - [ ] OCR 처리 최적화 (한국어 특화)
  - [ ] 테이블 구조 보존 품질 개선
- [x] **Marker 변환 엔진 기본 구현** (완성도: 85%)
  - [x] PDF 전용 고품질 변환
  - [x] 수식 및 도표 처리
  - [x] 레이아웃 보존 마크다운 변환
  - [x] 성능 최적화 및 배치 처리
  - [ ] 한국어 OCR 성능 개선 (장기 과제)
- [/] **문서 변환 API 부분 구현** (완성도: 70%)
  - [x] `/files/convert/marker`, `/files/convert/docling` 엔드포인트
  - [x] `/files/convert/compare` 비교 엔드포인트
  - [x] 변환 엔진 자동 선택 로직
  - [ ] `/v1/convert` 표준 엔드포인트 구현
  - [ ] 변환 진행률 추적 API
  - [ ] 변환 결과 캐싱 시스템
- [ ] **외부 서비스 연동 준비** (완성도: 0%)
  - [ ] Datalab Marker 서비스 연동 인터페이스
  - [ ] 변환 서비스 추상화 레이어
  - [ ] 외부 API 인증 및 보안 처리
...
#### 4. 문서 변환 시스템 완성 - 남은 작업들
- [ ] **표준 변환 API 구현** (우선순위: HIGH)
  - [ ] `/v1/convert` 표준 엔드포인트 구현
  - [ ] OpenAPI 3.0 스펙 준수
  - [ ] 변환 진행률 추적 API (`/v1/convert/{job_id}/status`)
  - [ ] 비동기 변환 작업 큐 시스템
- [ ] **변환 결과 캐싱 시스템** (우선순위: MEDIUM)
  - [ ] 문서 해시 기반 캐싱 로직
  - [ ] Redis/메모리 기반 캐시 저장소
  - [ ] 캐시 만료 및 정리 정책
  - [ ] 캐시 히트율 모니터링
- [x] **Office 문서 지원 완성** (완료!)
  - [x] DOCX, PPTX, XLSX 변환 기능 구현
  - [x] Docling 네이티브 Office 문서 지원 활용
  - [x] 자동 변환 엔진 선택 로직 (PDF→Marker, Office→Docling)
  - [x] 범용 문서 변환 인터페이스 구현
- [ ] **외부 서비스 연동** (우선순위: LOW)
  - [ ] Datalab Marker 서비스 연동 인터페이스
  - [ ] 변환 서비스 추상화 레이어
...
#### 10. 🌐 MCP (Model Context Protocol) 지원 (우선순위 하향 조정)
- [ ] **MCP 서버 구현**

...
- [x] **통합 업로드 API** - 업로드+처리 원스톱 API 구현 완료
- [x] **Marker 기본 변환** - PDF/DOCX/PPTX 모든 형식 지원 완료
- [ ] 문서 CRUD API 확장
- [ ] 문서 목록 및 필터링
- [ ] 문서 삭제 및 업데이트
- [ ] 벡터 인덱스 관리
...
- [x] **완전한 파이프라인 테스트** - 업로드+처리+검색 통합 테스트 완료
- [x] **모든 검색 타입 테스트** - 벡터/텍스트/하이브리드/리랭킹 검증 완료
- [ ] 단위 테스트 확장
- [ ] 부하 테스트 시나리오
- [ ] 테스트 데이터 관리
- [ ] CI/CD 파이프라인 통합
...
**마지막 업데이트**: 2025-07-15 - 통합 업로드 API 및 Marker 기본 변환 완료
**다음 검토 예정**: Sprint 3 시작 시

## 🎉 최근 완료된 기능

### ✅ 통합 업로드 API & Marker 기본 변환 (2025-07-15)
- **`/v1/upload_and_process` API**: 파일 업로드와 처리를 한 번에 수행하는 통합 엔드포인트
- **Marker 기본 변환**: PDF, DOCX, PPTX 모든 형식에 대해 marker를 기본 변환 방법으로 설정
- **완전한 파이프라인 테스트**: 업로드→처리→검색 전체 과정을 검증하는 종합 테스트 스크립트
- **KURE-v1 1024차원 지원**: Qdrant 벡터 차원 호환성 완전 해결
- **영문 파일명 정리**: 한글 샘플 문서들을 영문 파일명으로 변경하여 테스트 편의성 향상

**새로운 기능**:
- 단일 API 호출로 파일 업로드부터 벡터 저장까지 완전 자동화
- marker의 우수한 변환 품질을 모든 문서 형식에 적용
- 벡터/텍스트/하이브리드 검색 모든 케이스 자동 테스트
- 리랭킹 적용/미적용 성능 비교 테스트
- 응답 시간 및 검색 품질 실시간 모니터링
